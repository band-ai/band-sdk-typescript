...
const md = [
  ...
  "",
  `Generated by \`scripts/generate-c5-migration-doc.mjs\` from the authoritative map (\`c5-migration-map.json\`) and the before (\`${before.package}\` @ \`70a2822\`) / after (\`${after.package}\`) export surfaces. Single source of truth; cannot drift.`,
  "",
  "## Package",
  "",
  ...
  "|---|---|---|---|",
  ...map.symbols.map((s) => `| \`${s.old}\` | \`${s.new}\` | \`${s.subpath}\` | ${s.kind} |`),
  "",
  "## Public members (option properties)",
  "",
  "| Old member | 1.0 member | Subpath | Provenance |",
  "|---|---|---|---|",
  ...map.members.map((m) => `| \`${m.ownerOld}.${m.memberOld}\` | \`${m.ownerNew}.${m.memberNew}\` | \`${m.subpath}\` | ${m.provenance === "published-0x" ? "published 0.x" : "source-only"} |`),
  "",
  ...
  "",
  `Validation: ${map.symbols.length} symbol rows + ${map.members.length} member rows (${map.members.filter((m) => m.provenance === "published-0x").length} published 0.x, ${map.members.filter((m) => m.provenance === "source-c4").length} source-only) checked against the before/after surfaces (errors: **${errors.length}**). Residual \`Thenvoi\` exports in after: **0**. Member migrations are verified by compile fixtures and this repository's export-surface checks.`,
  "",
].join("\n");
