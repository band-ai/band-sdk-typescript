...
Release notes are maintained per package, and are generated by release-please:

- **`@band-ai/sdk`** — [`packages/sdk/CHANGELOG.md`](packages/sdk/CHANGELOG.md)
- **`@band-ai/openclaw-channel-band`** — [`packages/openclaw/CHANGELOG.md`](packages/openclaw/CHANGELOG.md)

This file is not generated and is no longer updated. It is kept only for the
pre-0.1.3 history below, which predates the move to per-package changelogs and
is not reproduced in them.

---

## Archive: pre-0.1.3 history
